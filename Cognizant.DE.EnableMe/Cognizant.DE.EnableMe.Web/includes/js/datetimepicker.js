...
;(function (root, factory) {
    ...
    function DirectiveController ($scope, $element, $attrs) {
      ...
      function minuteModelFactory (milliseconds) {
        ...
        var result = {
          ...
          'previousViewDate': new DateObject({
            utcDateValue: previousViewDate.valueOf(),
            display: selectedDate.startOf('day')
          }),
      ...
      function setTime (milliseconds) {
		...
		switch (configuration.modelType) {
          case 'Date':
            console.log('nothing');// No additional work needed
            break
          case 'moment':
            console.log('moment');newDate = moment([tempDate.getUTCFullYear(), tempDate.getUTCMonth(), tempDate.getUTCDate(), tempDate.getUTCHours(), tempDate.getUTCMinutes(), tempDate.getUTCSeconds(), tempDate.getUTCMilliseconds()])
            break
          case 'milliseconds':
            console.log(milliseconds);newDate = milliseconds
            break
          default: // It is assumed that the modelType is a formatting string.
            console.log('default');newDate = moment([tempDate.getUTCFullYear(), tempDate.getUTCMonth(), tempDate.getUTCDate(), tempDate.getUTCHours(), tempDate.getUTCMinutes(), tempDate.getUTCSeconds(), tempDate.getUTCMilliseconds()]).format(configuration.modelType)
        }
			console.log(newDate)
        var oldDate = ngModelController.$modelValue
		//var tesDate = newDate.toUTCString().split(' ').slice(0, 4).join(' ')
  ...
  function DateTimePickerConfigProvider () {
    var defaultConfiguration = {
      ...
      minView: 'minute',
      modelType: 'Date',
      parseFormat: 'yyy-mm-ddTHH:mm:ss.SSSZZ',
      renderOn: null,
      startView: 'day'
    }
