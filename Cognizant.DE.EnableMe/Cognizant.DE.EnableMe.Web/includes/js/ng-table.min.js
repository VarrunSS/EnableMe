...
app.factory('ngTableParams', ['$q', '$log', function ($q, $log) {
    ...
    var ngTableParams = function (baseParameters, baseSettings) {
        ...
        this.nextCount = function(count){
            var currentCountIndex = this.settings().counts.indexOf(count);
            if (currentCountIndex === this.settings().counts.length)
                return this.settings().counts[currentCountIndex-1];
            return this.settings().counts[currentCountIndex];
        };
        ...
        this.showingMax = function (){
            if (this.total() < this.page() * this.count())
                return this.total();
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pager.html', '<div class="ng-cloak ng-table-pager"> <div ng-if="params.settings().counts.length" class="ng-table-counts btn-group"> <button ng-repeat="count in params.settings().counts" type="button" ng-if="params.nextCount(count) < params.settings().total || count < params.settings().total" ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" class="btn btn-default countPage"> <span ng-bind="count"></span> </button> <div class="ng-table-showing tableInfo pull-left">Showing <span class="minCount" ng-bind="params.showingMin()"></span> to <span class="maxCount" ng-bind="params.showingMax()"></span> of <span class="totalCount" ng-bind="params.settings().total"></span> entries</div> </div> <ul class="pagination ng-table-pagination pull-right"> <li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" class="prevPagination" ng-switch="page.type"> <a ng-switch-when="prev" ng-click="params.page(page.number)" href="">&lt;</a> <a ng-switch-when="first" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="page" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a> <a ng-switch-when="last" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="next" ng-click="params.page(page.number)" class="nextPagination" href="">&gt;</a> </li> </ul> </div> ');
}]);
    return app;
}));